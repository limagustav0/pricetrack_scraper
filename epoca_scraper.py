    ...
    async with async_playwright() as p:
        ...
        for idx, produto in enumerate(produtos):
            try:
                ...
                # Extrair dados com retries para preço
                max_retries_price = 3
                retry_count = 0
                preco_final = "0"
                resultado = None

                while retry_count < max_retries_price and preco_final == "0":
                    if retry_count > 0:
                        logger.info("[Época] Preço final '0' detectado. Tentativa %d/%d após espera.", retry_count + 1, max_retries_price)
                        await asyncio.sleep(random.uniform(2, 5))

                    # Reabrir página de detalhes se necessário
                    if retry_count > 0:
                        await detail.close()
                        detail = await context.new_page()
                        await detail.goto(link, timeout=30000)
                    ...
                    preco_final = str(json_data.get("offers", {}).get("price", "0"))
